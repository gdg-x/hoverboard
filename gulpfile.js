...
gulp.task('images', function() {
    return gulp.src('app/images/**/*')
        ...
        .pipe($.size({
            title: 'Copy optimized images to dist/images dir:'
        }));
});
...
gulp.task('fonts', function() {
    return gulp.src(['app/themes/' + config.theme + '/fonts/**/*.{css,woff,woff2}'])
        ...
        .pipe($.size({
            title: 'Copy fonts to dist/themes/' + config.theme + '/fonts dir:'
        }));
});
...
gulp.task('html', ['views'], function() {
    ...
    return gulp.src(['app/*.html', '.tmp/*.html'])
        ...
        .pipe(gulp.dest('dist'))
        .pipe($.size({
            title: 'Copy optimized html and assets files to dist dir:'
        }));
});
...
gulp.task('vulcanize', function() {
    return gulp.src('dist/elements/elements.vulcanized.html')
        ...
        .pipe(gulp.dest('dist/elements'))
        .pipe($.size({
            title: 'Copy vulcanized elements to dist/elements dir:'
        }));
});
...
gulp.task('cache-config', function(callback) {
    ...
    return glob('{elements,scripts,themes}/**/*.*', {
        cwd: dir
...
gulp.task('serve', ['images', 'styles', 'js', 'lint', 'manifest', 'views'], function() {
    browserSync({
    ...
    gulp.watch(['app/{elements,themes}/**/*.{css,html}'], ['styles', 'views', reload]);
    gulp.watch(['app/{scripts,elements}/**/*.{js,html}'], ['jshint', 'js', 'views']);
...
gulp.task('default', ['clean'], function(cb) {
    runSequence(
        ['copy', 'styles', 'lint', 'js', 'manifest'], ['jshint', 'images', 'fonts', 'html'],
        'vulcanize', ['clean-dist', 'minify-dist'],
        'cache-config',
        cb);
});
